...
const Assignment = () => {
	const {getAllClasses, allClasses, getSubjects, allSubjects, addAssignment, getAssignment, assignments, postComment, getComments, allComments} = useTeacherAuth()
	...
	const {register, handleSubmit, formState: {errors}} = useForm({resolver: yupResolver (addAsgtSchema)})
	
	const onSubmit = (data) => {
		addAssignment(data, setIsModalOpen);
		console.log(data)
		};
		...
		<div className="assignment-screen">
			...
			<article>
				...
				<div>
					...
					<select name="class" id="class" onChange={(e) => setClassID(e.target.value)}>
				...
				<div>
					...
					{allClasses.map((cls) => (
						<option key={cls.id} value={cls.id}>{cls.class_teacher}</option>
					))}
					</select>
				</div>

				<button onClick={() => {getAssignment(classID, teacherID, date);}} className="primary-btn">
					Search
				</button>
			...
			<main>
				{
					...
					<div className="top">
						...
						<span onClick={() => {setIsModal2Open(true); getComments(assignment.id); setCommentId(assignment.id) }}>
							<LiaCommentDots />
							<p>Add Comments</p>
						</span>
					</div>
					...
					<form onSubmit={handleSubmit(onSubmit)}>
						<article>
							...
							<div className="form-group">
								...
								<select {...register("teacher_id")} onChange={(e) => setTeacherId(e.target.value)}>
			...
			<Modal isOpen={isModal2Open} onClose={closeModal2}>
                ...
                type="button" onClick={() => postComment(commentId, comment, setIsModal2Open)}
